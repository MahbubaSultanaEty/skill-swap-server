...
async function ensureConnected() {
  ...
  collections = {
    ...
    proposalCollection: database.collection("proposals"),
    plansCollection: database.collection("plans"),
...
// Middleware: ensures DB connection exists before any route runs
...
const verifyToken = async (req, res, next) => {
  ...
  try {
    ...
    const user = await collections.userCollection.findOne({ email: payload.email });
    if (!user) {
...
app.get("/api/users", async (req, res) => {
  ...
  const result = await collections.userCollection.find(users).toArray();
  res.send(result);
});
...
app.get("/api/users/email/:email", async (req, res) => {
  const user = await collections.userCollection.findOne({ email: req.params.email });
  if (!user) {
...
app.get("/api/freelancers", async (req, res) => {
  const freelancers = await collections.userCollection
...
app.post("/api/tasks", verifyToken, async (req, res) => {
  const task = req.body;
  const result = await collections.taskCollection.insertOne(task);
  res.send(result);
});
...
app.get("/api/tasks", async (req, res) => {
  ...
  const tasks = await collections.taskCollection
    .find(query)
    .skip(skip)
    .limit(perPage)
    .toArray();

  const total = await collections.taskCollection.countDocuments(query);
...
app.get("/api/tasks/currentTask/:id", async (req, res) => {
  ...
  const result = await collections.taskCollection.findOne({ _id: new ObjectId(id) });
  res.send(result);
});
...
app.get("/api/tasks/:clientId", async (req, res) => {
  ...
  const result = await collections.taskCollection.find(query).toArray();
  res.send(result);
});
...
app.patch("/api/tasks/:id/status", verifyToken, async (req, res) => {
  ...
  const result = await collections.taskCollection.updateOne(
    { _id: id },
    { $set: updateFields }
  );
  res.send(result);
...
app.delete("/api/tasks/:id", verifyToken, async (req, res) => {
  ...
  const result = await collections.taskCollection.deleteOne({
    _id: new ObjectId(id),
  });
...
app.patch("/api/proposals/:id/status", verifyToken, async (req, res) => {
  ...
  const result = await collections.proposalCollection.updateOne(
    { _id: new ObjectId(id) },
    { $set: updateFields }
  );
  res.send(result);
...
app.get("/api/proposals", verifyToken, async (req, res) => {
  ...
  const proposals = await collections.proposalCollection
    .find(query)
    .sort({ submittedAt: -1 })
    .toArray();
...
app.post("/api/proposals", verifyToken, verifyFreelancer, async (req, res) => {
  ...
  const result = await collections.proposalCollection.insertOne(newProposal);
  res.send(result);
});
...
app.get("/api/plans", async (req, res) => {
  ...
  const plan = await collections.plansCollection.findOne(query);
  res.send(plan);
});
...
app.post("/api/subscriptions", async (req, res) => {
  ...
  const result = await collections.subscriptionsCollection.insertOne(subInfo);

  const filter = { email: data.email };
  const updateDocument = {
    $set: { plan: data.planId },
  };
  const updatedResult = await collections.userCollection.updateOne(
    filter,
    updateDocument
  );
...
app.post("/api/reviews", verifyToken, verifyClient, async (req, res) => {
  ...
  const result = await collections.reviewCollection.insertOne(review);
  res.send(result);
});
...
app.get("/api/payments", verifyToken, async (req, res) => {
  ...
  const payments = await collections.paymentCollection
    .find(query)
    .sort({ paid_at: -1 })
    .toArray();
...
app.post("/api/payments", verifyToken, async (req, res) => {
  ...
  const result = await collections.paymentCollection.insertOne(payment);
  res.send(result);
});

// Local dev only — Vercel ignores this and uses module.exports
